...
No tests or linter. The only build step is the frontend: `scripts/build-frontend.js` compiles `public/*.jsx` → `public/build/*.js` (same Babel options the browser used to apply: presets react + env), generates Tailwind CSS from `tailwind.config.js`, and copies the React production UMD builds. `public/build/` is gitignored and regenerated on every deploy (`npm run build`) and on every restart of `npm run dev` (nodemon watches .jsx/.html too). Editing a .jsx without rebuilding has no effect.
...
### Frontend — React without bundler

Each .jsx is compiled on its own into a classic script (no bundling, no modules), so files still share globals exactly as before. Tailwind classes must be written whole in the source (no `bg-${color}-500`): the CSS is generated by scanning the files, not at runtime.

`public/index.html` loads in order (as `/build/<name>.js`):
